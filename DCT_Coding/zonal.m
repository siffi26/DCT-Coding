...
%%Reading .raw file
function [y,d]= zonal(filename, step_size, coefficents)
if true
    row = 512; col=512;
X = fopen(filename, 'r');
...
%%Entropy Calculation
size =(4096/step_size);
Entropy =zeros(10000, 1);

%%Entropy calculation
for i= 1:64
    for j=1:64
        for n = 1:coefficents
           Entropy(uint8(Coeff(i,j,n))+2048)=Entropy(uint8(Coeff(i,j,n))+2048)+1;
        end
    end
end
Entropy=double(Entropy)/size;
Entropy
sum(Entropy)
Total_Entropy=0;
for i= 1:size
    if(Entropy(i))
        Total_Entropy=Total_Entropy+(-Entropy(i)*log(Entropy(i)));
    end
end
Total_Entropy
